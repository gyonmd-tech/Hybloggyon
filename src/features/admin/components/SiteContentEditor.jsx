...
function HomeFields({ data, setData }) {
  return <>
    ...
    <RepeatableList data={data} setData={setData} path={['hobbies']} title="Minat & obsesi" fields={[{ key: 'label', label: 'Kategori' }, { key: 'tag', label: 'Tag' }, { key: 'title', label: 'Judul' }, { key: 'imageUrl', label: 'Gambar', help: urlHelp }, { key: 'description', label: 'Deskripsi', type: 'textarea' }]} createItem={() => ({ label: '', title: '', description: '', tag: '[ NEW ]', imageUrl: '/images/og/default.webp' })} />
    <RepeatableList data={data} setData={setData} path={['showcase']} title="Showcase / Koleksi & arsip" fields={[{ key: 'title', label: 'Judul' }, { key: 'tag', label: 'Keterangan' }, { key: 'imageUrl', label: 'Gambar', help: urlHelp }]} createItem={() => ({ title: '', tag: '', imageUrl: '/images/og/default.webp' })} />
    <RepeatableList data={data} setData={setData} path={['logs']} title="Log observasi" fields={[{ key: 'date', label: 'Tanggal' }, { key: 'time', label: 'Waktu', required: false }, { key: 'status', label: 'Status', type: 'select', options: [{ value: 'OPEN', label: 'Open' }, { value: 'CLOSED', label: 'Closed' }] }, { key: 'text', label: 'Catatan', type: 'textarea' }]} createItem={() => ({ date: '', time: '', text: '', status: 'OPEN' })} />
    <RepeatableList data={data} setData={setData} path={['timeline']} title="Jejak & evolusi" fields={[{ key: 'year', label: 'Tahun' }, { key: 'title', label: 'Judul' }, { key: 'description', label: 'Deskripsi', type: 'textarea' }]} createItem={() => ({ year: String(new Date().getFullYear()), title: '', description: '' })} />
  </>;
}

function AboutFields({ data, setData }) {
  return <>
    ...
    <RepeatableList data={data} setData={setData} path={['meta']} title="Metadata profil" fields={[{ key: 'label', label: 'Label' }, { key: 'value', label: 'Nilai' }]} createItem={() => ({ label: '', value: '' })} />
    <TextList data={data} setData={setData} path={['manifestoParagraphs']} title="Paragraf manifesto" />
    <TextList data={data} setData={setData} path={['beliefs']} title="Prinsip / beliefs" />
    <RepeatableList data={data} setData={setData} path={['socialLinks']} title="Tautan sosial" fields={[{ key: 'label', label: 'Platform' }, { key: 'url', label: 'URL' }]} createItem={() => ({ label: '', url: 'https://' })} />
  </>;
}

function HobbyFields({ data, setData }) {
  return <>
    ...
    <RepeatableList data={data} setData={setData} path={['music']} title="Musik" fields={[{ key: 'artist', label: 'Artis' }, { key: 'title', label: 'Judul' }, { key: 'year', label: 'Tahun' }, { key: 'genre', label: 'Genre' }, { key: 'mood', label: 'Mood' }, { key: 'isCurrentlyPlaying', label: 'Sedang diputar', type: 'checkbox' }]} createItem={(index) => ({ id: Date.now() + index, artist: '', title: '', year: '', genre: '', mood: '', isCurrentlyPlaying: false })} />
...
function NotesFields({ data, setData }) {
  return <>
    ...
    <TextList data={data} setData={setData} path={['randomThoughts']} title="Random thoughts" />
    <RepeatableList data={data} setData={setData} path={['connections']} title="Benang merah antarcatatan" fields={[{ key: 'from', label: 'Node A' }, { key: 'to', label: 'Node B' }, { key: 'via', label: 'Hubungan', type: 'textarea' }]} createItem={() => ({ from: '', to: '', via: '' })} />
  </>;
}

const textListPaths = { about: [['manifestoParagraphs'], ['beliefs']], notes: [['randomThoughts']] };

export default function SiteContentEditor({ section, initialContent }) {
